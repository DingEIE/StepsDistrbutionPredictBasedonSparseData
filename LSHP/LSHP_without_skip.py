...
class LSHP(nn.Module):
    def __init__(self) -> None:
        ...
        self.Conv2 = nn.Conv2d(1, 1, kernel_size=(3, 1), stride=(3, 1), bias=True)
    ...
    def forward(self, Conv1_input, dataYweek_std, dataYhour, linear_input):
        ...
        for day in range(n_steps_out):
            ...
            conv2_input = torch.cat((rnn1_out, dataYweek, dataYhour), 2).to(torch.float32)
...
def train(lshp, input_Conv1, dataYweek, dataYhour, linear_input, y_label):
    ...
    save_path = "./model/"
    mkdir(save_path)
    torch.save(lshp.state_dict(), save_path+"lshp-without-skip-"+str(scale_val)+".pt")
    print("save done")
...
if __name__ == "__main__":
    save_path = "./model/"
    filepath = save_path+"lshp-without-skip-"+str(scale_val)+".pt"
    lshp = LSHP()
