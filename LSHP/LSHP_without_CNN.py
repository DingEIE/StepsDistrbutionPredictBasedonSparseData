...
class LSHP(nn.Module):
    def __init__(self) -> None:
        ...
        self.data = DataUtils()
        self.Conv1 = nn.Linear(3, 1)    # 原本的conv转为线性层
    ...
    def forward(self, Conv1_input, dataYweek_std, dataYhour, linear_input):
        ...
        for day in range(n_steps_out):
            ...
            Conv1_input1 = Conv1_input.permute(0, 1, 3, 2)
            conv1_out = torch.tanh(torch.squeeze(self.Conv1(Conv1_input1)))
            ...
            results_day = self.norm6(results_day)
            # print(results_day.shape)
...
def train(lshp, input_Conv1, dataYweek, dataYhour, linear_input, y_label):
    ...
    save_path = "./model/"
    mkdir(save_path)
    torch.save(lshp.state_dict(), save_path+"lshp-without-cnn-"+str(scale_val)+".pt")
    print("save done")
...
if __name__ == "__main__":
    save_path = "./model/"
    filepath = save_path+"lshp-without-cnn-"+str(scale_val)+".pt"
    lshp = LSHP()
