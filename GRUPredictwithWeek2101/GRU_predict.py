...
# 定义函数用于GRU模型训练
# 函数输入： std_x_steps_train, std_x_week_train, std_y_week_train, std_y_steps_train
# 函数输出：save_path
def trainModel(std_x_steps_train, std_x_week_train, std_y_week_train, std_y_steps_train):
    # 配置训练模型
    # 使用GRU模型
    ...
    batch_size = 2000
    std_x_data = np.concatenate((std_x_steps_train, std_x_week_train), axis=1)
    if os.path.exists(weight_save_path):
        print(weight_save_path+' has exist! ')
    else:
        # 输入预测函数
        model.fit([std_x_data, std_y_week_train], [std_y_steps_train], batch_size=batch_size, epochs=epochs, verbose=1)
        # 对模型进行存储
...
# 定义函数用于预测参数
# 函数输入：(GRU_model, std_x_steps_test, std_x_week_test, std_y_week_test, std_y_steps_test, std_y_week)
# 函数输出: smape_val, rmse_val
def predictData(GRU_model, std_x_steps_test, std_x_week_test, std_y_week_test, std_y_steps_test, std):
    std_x_data_test = np.concatenate((std_x_steps_test, std_x_week_test), axis=1)
    # 预测数据
    trainPredict_std = GRU_model.predict([std_x_data_test, std_y_week_test])
...
if __name__ == "__main__":
    ...
    y_week = np.load(filePath_save_y_week).reshape(-1, time_num*n_steps_out)
    ...
    x_week_std = std_x_week.fit_transform(x_week)
    y_week_std = std_y_week.fit_transform(y_week)
    x_steps_std = x_steps_std.reshape(-1, 1, n_steps_in*time_num)
    x_week_std = x_week_std.reshape(-1, 1, n_steps_in*time_num)
    ...
    # 分割数据集为测试集和训练集
    std_x_steps_train, std_x_steps_test, std_x_week_train, std_x_week_test, std_y_week_train, std_y_week_test, std_y_steps_train, std_y_steps_test = train_test_split(x_steps_std, x_week_std, y_week_std, y_steps_std, test_size=0.2, random_state=1)

    # # 输入预测函数
    # trainModel(std_x_steps_train, std_x_week_train, std_y_week_train, std_y_steps_train)
    ...
    #     # 输入预测函数
    #     trainModel(std_x_steps_train, std_x_week_train, std_y_week_train, std_y_steps_train)
    GRU_model = trainModel(std_x_steps_train, std_x_week_train, std_y_week_train, std_y_steps_train)
    # 加载预测模型
    
    smape_val, rmse_val, r2 = predictData(GRU_model, std_x_steps_test, std_x_week_test, std_y_week_test, std_y_steps_test, std_y_steps)
    print("The SMAPE is "+str(smape_val*100)+"%. ")
    print("The rmse is "+str(rmse_val))
    print("The r2 score is "+str(r2))

    filesPath_x = "./XY/x_data/"
    filesPath_y_week = "./XY/y_week/"
    filesPath_y_steps = "./XY/y_steps/"
    files_x = os.listdir(filesPath_x)
    files_y_week = os.listdir(filesPath_y_week)
    ...
    for i in range(len(files_x)):
        ...
        file_y_week = filesPath_y_week+files_y_week[i]
        ...
        for i in range(data_x.shape[0]):
            ...
            x_week.append(data_x[i][1])
        x_steps = np.array(x_steps).reshape(-1, n_steps_in*time_num)
        x_week = np.array(x_week).reshape(-1, n_steps_in*time_num)
        data_y_week = np.load(file_y_week).reshape(-1, n_steps_out*time_num)
        data_y_steps = np.load(file_y_steps).reshape(-1, n_steps_out*time_num)
        smape_val, rmse_val, r2, r = predictDataEvery(GRU_model, x_steps, x_week, data_y_week, data_y_steps)
        result_df = result_df.append({'ID':file_name, 'SMAPE':smape_val, "RMSE":rmse_val, "R2":r2, "R":r}, ignore_index=True)
        # print(result_df)
    result_df.to_csv("results_every_ID_withweek.csv", index=False)